...
test("permanent runtime image is digest-pinned, non-root and contains no secret or release config", () => {
  ...
  assert.match(source, /--mount=type=bind,from=npm-cache,source=\.,target=\/npm-cache,ro/);
  assert.match(source, /npm ci --ignore-scripts --omit=dev --offline --cache=\/npm-cache/);
