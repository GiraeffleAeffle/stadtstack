...
- **Status:** accepted; staff HTTP/coordinator/SQLite and pinned runtime integration implemented; deployment activation pending
- **Date:** 2026-09-05
...
The runtime's optional JSON `citizenAdoption` configuration contains the
verified `policy` and one `acceptanceBaseUrl`. Loopback, reviewed Operations and
recovery compositions pass the same captured inputs to the existing writer
and select the adoption HTTP contract. Municipality, policy version and agent
keys must match the runtime. The citizen allowlist may be empty in this lane;
the pinned issuer's current signed eligibility identifies eligible citizens.
Legacy deployments retain their original request contract and nonempty list.

The trusted acceptance Adapter appends only the lowercase event ID to the
configured HTTPS base path, sends no credentials and rejects redirects. Both
ledger and signed status responses have a 16 KiB JSON limit and share the
verifier's 10-second abort budget. Requests cannot override endpoints or inject
functions, acceptance, status or signing keys into this JSON configuration.
The issuer owns credential verification and all private holder/signing material.

For Röbel, the acceptance base path is
`/api/staging-participant/v1/citizen-adoption/acceptance`; its deployment must
first install the independently reviewed exact-event RPC and permissions.
The runtime is implemented but not activated by a source merge. Reviewed
endpoint/key pins, the issuer status deployment, an eligibility operator and a
real municipal Case Steward remain prerequisites for the live journey.
